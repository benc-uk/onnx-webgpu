...
export async function GetCapability() {
  ...
  try {
    ...
    // Debugging
    const feats = {}
    for (const [key, value] of adapter.features.entries()) {
      feats[key] = true
    }
...
export async function checkCache(urlString, storeName = 'onnx') {
  const cache = await caches.open(storeName)

  const keys = await cache.keys()

  for (const cacheKeyReq of keys) {
    if (cacheKeyReq.url.includes(urlString)) {
      console.log(`### ${urlString} Found in cache`)
      return true
    }
  }
